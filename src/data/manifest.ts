...
import { ExportRecipe, ExportResource, ExportWeapon } from "./schema";
...
type Exports = Record<Exclude<ExportCategory, keyof TypedExports>, object> &
  TypedExports;
...
export class Manifest {
  ...
  image_url = (name: string) =>
    `http://content.warframe.com/PublicExport/${this.textures[name]}`;

  static async create() {
    const url = "https://origin.warframe.com/PublicExport/index_en.txt.lzma";

    const self = new Manifest();
    ...
    const lines = new TextDecoder()
      ...
      .map((l) => Manifest.get_export_text(l));
    ...
    for (const weapon of self.exports["ExportWeapons"])
      self.names[weapon.uniqueName] = weapon.name;

    for (const k of self.exports["ExportResources"])
      self.names[k.uniqueName] = k.name;
